...
| **D-002** | ~~First driver target is the **most common VM engines** (KVM/QEMU, VirtualBox, VMware, Firecracker, Proxmox, then AWS/GCP/Azure)~~ → **Parked** alongside Core (2026-09-20, P8). The Runtime runs on Linux; the driver/VM-engine target is a Core-track concern, reactivated only if Core is. | `virtio` is the common denominator; VM-first gives a short, well-documented driver list and avoids the hardest desktop-hardware problems (GPU, Wi-Fi, suspend). |
...
| **D-006** | ~~**IOMMU required**; hardware without IOMMU is unsupported~~ → **Parked** alongside Core (2026-09-20, P8). It governs driver isolation on bare metal/Core; the Runtime's DMA isolation is owned by the Linux host kernel. | A device can only DMA into buffers it was explicitly given; without an IOMMU the driver-isolation model does not hold. |
...
| **D-014** | **First target niche is the agent execution host** | The machine you run agents on, where what an agent touched is a queryable fact and what it could touch was bounded before it started. Headless; "server host" dropped. *Host* is the niche — the machine; *platform* is the product category used in the one-line definition. The two are deliberate, not drift. See [`0001`](../rfcs/0001-target-niche.md). |
